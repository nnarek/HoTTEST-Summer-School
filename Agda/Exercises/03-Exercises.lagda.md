...
is-lower-bound P n = Π m ꞉ ℕ , (P m → n ≤₁ m)
```
...
is-minimal-element-suc P d m pm is-lower-bound neg-p0 zero pn = neg-p0 pn
...
well-ordering-principle P d (suc n) p = well-ordering-principle-suc P d n p (d 0) (well-ordering-principle (λ z → P (suc z)) (λ x → d (suc x)) n p)
```
...
is-zero-well-ordering-principle P d (suc m) pm =
  is-zero-well-ordering-principle-suc P d m pm (d 0) (well-ordering-principle (λ z → P (suc z)) (λ x → d (suc x)) m pm)
``` 
