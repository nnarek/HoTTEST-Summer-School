...
!-example : 4 ! ≡ 24
!-example = refl _ -- refl 24 should fill the hole here
```
...
min-example = refl _ -- refl 3 should fill the hole here
```
...
```agda
≡-to-≣ : (a b : Bool) → a ≡ b → a ≣ b
≡-to-≣ a b (refl .a) = Bool-refl a

≣-to-≡ : (a b : Bool) → a ≣ b → a ≡ b
≣-to-≡ true true h = refl _
≣-to-≡ false false h = refl _
```
...
map-id [] = refl _
map-id (x :: xs) = ap₂ _::_ (refl _) (map-id xs)

map-comp : {X Y Z : Type} (f : X → Y) (g : Y → Z)
           (xs : List X) → map (g ∘ f) xs ≡ map g (map f xs)
map-comp f g [] = refl _
map-comp f g (x :: xs) = ap₂ _::_ (refl _) (map-comp f g xs)
```
